...
async def handle_deepgram_streaming(websocket: WebSocket):
    """WebSocket handler for Deepgram using fake streaming (buffered approach)."""
    ...
    try:
        ...
        print(f"Started Deepgram buffered STT session: {session_id}")
            ...
            try:
                ...
                if audio_buffer_manager.should_process_buffer(session_id, current_time):
                    ...
                    try:
                        ...
                        print(f"Deepgram transcription result: '{result.text}' (confidence: {result.confidence})")
                        ...
                        if result.text.strip():
                            ...
                            print(f"Sending Deepgram WebSocket response: {response_data}")
                            await websocket.send_json(response_data)
                        else:
                            print("Deepgram transcription result was empty, not sending")
                            
                    except Exception as transcription_error:
                        print(f"Error transcribing buffered audio with Deepgram: {transcription_error}")
                        traceback.print_exc()
                ...
                print(f"Error in Deepgram audio processing loop: {loop_error}")
                traceback.print_exc()
                break
        ...
        if len(final_audio) > 0:
            try:
                ...
                print(f"Error processing final Deepgram audio buffer: {final_error}")
        
        audio_buffer_manager.cleanup_session(session_id)
        print(f"Cleaned up Deepgram buffering session: {session_id}")
