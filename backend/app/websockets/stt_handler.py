...
async def handle_stt_websocket(websocket: WebSocket):
    ...
    if provider == "soniox":
        ...
        print("Routing to Deepgram fake streaming handler")
        await handle_deepgram_streaming(websocket)
    else:
        # Fallback to Deepgram approach for other providers
        print(f"Unknown provider '{provider}', falling back to Deepgram-style streaming")
        await handle_deepgram_streaming(websocket)
